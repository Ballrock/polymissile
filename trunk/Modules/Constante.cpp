...
void Constante::setTableColor(SDL_Surface *ecran)
{
	Constante::couleurs[C_NOIR]          = SDL_MapRGB(ecran->format, 0x00, 0x00, 0x00);
  	Constante::couleurs[C_BLEU_FONCE]    = SDL_MapRGB(ecran->format, 0x00, 0x00, 0x70);
  	Constante::couleurs[C_VERT_FONCE]    = SDL_MapRGB(ecran->format, 0x00, 0x70, 0x00);
  	Constante::couleurs[C_CYAN_FONCE]    = SDL_MapRGB(ecran->format, 0x00, 0x70, 0x70);
 	Constante::couleurs[C_ROUGE_FONCE]   = SDL_MapRGB(ecran->format, 0x70, 0x00, 0x00);
  	Constante::couleurs[C_MAGENTA_FONCE] = SDL_MapRGB(ecran->format, 0x70, 0x00, 0x70);
  	Constante::couleurs[C_OCRE]          = SDL_MapRGB(ecran->format, 0x70, 0x70, 0x00);
  	Constante::couleurs[C_GRIS_CLAIR]    = SDL_MapRGB(ecran->format, 0xC0, 0xC0, 0xC0);
  	Constante::couleurs[C_GRIS]          = SDL_MapRGB(ecran->format, 0x70, 0x70, 0x70);
